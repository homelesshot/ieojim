...
- **재녹음 경쟁 상태(race condition) 버그**: 변환 처리 중에 곧바로 재녹음을 시작하면, 뒤늦게 실패로 돌아온 이전 시도의 오류가 지금 진행 중인 새 녹음 화면을 덮어써 버튼이 먹통처럼 보이는 현장 신고 발생 → 요청을 구분하는 세대 번호를 녹음이 "끝났을 때"가 아니라 "새로 시작될 때" 즉시 올리도록 수정, 버튼 상태 판단 기준도 화면에 항상 정확히 반영되는 값으로 통일해 근본 해결.
